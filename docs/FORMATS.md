...
| **Asobo BigFile** | Wii / Archive | ✅ | ❌ | Asobo Studio "Internal Cross Technology" `.DRV` volumes (*Ratatouille*): big-endian v1.06.63 blocks, stored or LZRS members, hashed names (Asobo CRC) with class names (`Bitmap_Z`, `Mesh_Z`, ...); `Bitmap_Z` (RGB565 / RGBA8 / CMPR) → PNG. Format after widberg/bff |
...
| **Avalanche THB/TBB** | Wii / Texture | ✅ | ❌ | Avalanche Software engine textures (*Cars 2*, inside the game's `.zip` / `.tszip` archives): 48-byte big-endian `.thb` header + tiled GX `.tbb` pixels (I8 / RGB5A3 / RGBA8 / CMPR) → PNG. Multi-texture headers are not decoded |
