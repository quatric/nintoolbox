...
| **Blue Tongue TRB** | Wii / Archive | ✅ | ❌ | *de Blob 2* `.trb` (`TRB\0`, version 0x7d1, big-endian; not the TSFB layout of Barnyard): 0x30-byte section descriptors at 0x80 `{0, name offset, 0, flags, size, size, file offset}` (section 0 is the string table; `.data`, `__s00000` pixel pool, `gpu_data`, `model_collision`, `XUR`), then 16-byte symbols `{tag, offset, section << 16, name offset}`. A `ttex` symbol is a texture object: `0x0df00bb0` fill, then GX format (0 I4, 1 I8, 2 IA4, 3 IA8, 5 RGB5A3, 6 RGBA8, 8 C4, 9 C8, 14 CMPR), pixel offset in the pool at +0xc, palette offset +0x14 and format +0x18, stored size +0x1c, width / height pairs at +0x40 (+0x44 in UI files; first pair is the stored size) and the `.tga` name; only the base level is decoded. The `XUR` section is written as a `.xur` file. Models (`gpu_data`) and the `CETB` compressed level are not decoded. |
