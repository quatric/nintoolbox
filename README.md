...
| **COD PAK0** | `.pak` | ✅ | ✅ | ✅ | ✅ | *Call of Duty: Black Ops* / *MW3* (Wii) sound archive (`PAK0`). `wszst create` rebuilds a CRC-named `.dsp` tree: when the target `.pak` still holds the same members it reuses the original layout (header dword, multiplier, `DATASTART`, entry table) for a byte-exact round trip, otherwise it writes a fresh valid PAK0 (multiplier 1, CRC-sorted, members back to back). Verified against retail *Call of Duty: Black Ops* (Wii) WBFS disc: extracts members including `retail_int_escape_0x1e78d28c.dsp` audio stream and repacks `retail_int_escape.pak` byte-for-byte. Fixture: `tests/fixtures/wii_retail/retail_int_escape.pak` (4,096 bytes) |
...
| **Storybook ONE** | `.one` | ✅ | ✅ | — | ✅ | Sonic Team *Sonic and the Secret Rings* / *Black Knight* PRS-compressed container. The size-limit check used `out_size > opt_max_file_size` where 0 means "no limit" everywhere else in this codebase, so every real member (any nonzero size) was rejected unless `--max-file-size` happened to be set on the command line -- fixed and verified against a retail `.one` (33 members). `wszst CREATE` now writes ONE archives too: `CreateONEArchive()` (`lib-one.c`) compresses each member with the project's own greedy-LZ77 PRS encoder, whose bit stream the same reader accepts and round-trips every payload byte -- a content-exact envelope, not byte-identical to Sonic Team's compressor. Round-trip exercised by `tests/regress.sh` |
...
| **BCMDL / CGFX** | `.bcmdl`, `.cgfx` | **GLB** | ✅ | ✅ | ✅ | ✅ | NintendoWare NW4C CGFX 3D model resource (3DS). ENCODE roundtrips the fixture byte-exactly: when the GLB still matches the original (verified by re-parsing the embedded raw container and comparing geometry, material and skeleton identity through the GLB index mapping), the source bytes are returned verbatim; any real edit triggers a fresh canonical rebuild |
| **BCRES** | `.bcres` | **GLB** | ✅ | ✅ | ✅ | ✅ | NintendoWare NW4C CGFX 3D graphics and model resource container (3DS). Same GLB -> BCRES ENCODE as BCMDL: embedded-original passthrough while the model is unchanged (bit-for-bit), full structural rebuild once the GLB deviates from it |
...
| **NUS3AUDIO** | `.nus3audio`, `.nus3bank` | ✅ | ✅ | — | Bandai Namco NUS3 audio archive (*Super Smash Bros. Ultimate* Switch). `wszst create` names tracks by their TNNM entries (`.idsp` / `.lopus` / `.bnsf` / `.wav` via payload magic, else `.bin`); when the target archive still holds the same tracks it reuses the original layout (chunk order, TNID ids, TNNM table, PACK offsets, trailing data) and only overwrites the PACK payloads for a byte-exact round trip, otherwise it writes a fresh valid NUS3. Note: *Super Smash Bros. for Nintendo 3DS* retail `.nus3bank` files use a `BANKTOC ` (8-byte) chunk tag instead of the Switch/Wii U `AUDIINDX` tag; the current decoder doesn't recognise `BANKTOC` and returns `ERR_INVALID_DATA` — the 3DS NUS3 variant is **not supported** |
