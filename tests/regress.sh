...
t_cgfx(){
  ...
  if [ -f "$f" ]; then
    ...
    if [ "$g" -gt 0 ] 2>/dev/null; then
      ...
      # BcmdlImporter streams: the retail sample carries vertex colours on
      # all shapes, tangent + second UVs on one shape, and rigid skinning
      # palettes on every submesh -- all must survive into the GLB.
      local cgfx_streams; cgfx_streams=$(python3 -c "
import json, struct
d = open('/tmp/_r.glb', 'rb').read()
...
need = {'COLOR_0', 'TANGENT', 'TEXCOORD_1', 'JOINTS_0', 'WEIGHTS_0'}
missing = sorted(need - attrs)
skins = len(j.get('skins', []))
print(('MISSING:' + ','.join(missing)) if missing else ('NOSKIN' if skins < 1 else 'ALL'))
" 2>/dev/null || true)
      if [ "$cgfx_streams" = "ALL" ]; then
        ok "CGFX (3DS) color/tangent/uv1/skinning streams in GLB"
      else
        no "CGFX (3DS) color/tangent/uv1/skinning streams in GLB" "got '$cgfx_streams'"
      fi
      ...
      # The rebuilt container must re-encode the same extended streams, so
      # the second GLB carries them too (fresh-build path, not passthrough).
      local cgfx_rt; cgfx_rt=$(python3 -c "
import json, struct
d = open('/tmp/_r_roundtrip.glb', 'rb').read()
jl = struct.unpack('<I', d[12:16])[0]
j = json.loads(d[20:20 + jl])
attrs = set()
for m in j.get('meshes', []):
...
" 2>/dev/null || true)
      if [ "$cgfx_rt" = "ALL" ]; then
        ok "CGFX (3DS) rebuilt container keeps color/tangent/uv1/skinning"
      else
        no "CGFX (3DS) rebuilt container keeps color/tangent/uv1/skinning" "got '$cgfx_rt'"
      fi
      ...
      # resolving the accessor through the primitive (accessors[0] is the
      # skin inverse-bind matrices once skinned export is active).
...
print(struct.unpack_from('<1f', d, off)[0])" 2>/dev/null || true)
      ...
      # TextureMeta sidecar (BcmdlImporter port): every exported PNG must
      # have a .json describing the original PICA hardware format.
      local tjson; tjson=$(find /tmp/_r_cgfx_texdir -maxdepth 1 -name '*.json' 2>/dev/null | head -1)
      if [ -n "$tjson" ] && grep -q '"format_name"' "$tjson" 2>/dev/null; then
        ok "CGFX (3DS) texture format sidecars ($(basename "$tjson"))"
      else
        no "CGFX (3DS) texture format sidecars" "no .json with format_name beside textures"
      fi
      return
    fi
...
t_bntx_legacy(){
  # Ports from KillzXGaming/LegacySwitchLibraries (Bntx GFX/Enums.cs,
  # Bntx UserData.cs, BntxFile multi-texture list, Bfres ResFile lists):
  # EAC/ETC2/PVRTC format names, per-texture UserData STRING/WSTRING
  # string arrays, every-texture decode, and the Switch BFRES archive
  # census (FMDL/FSKA/FMAA/FBVS/FSHA/FSCN + external files).
  # Format-name asserts live in tests/test-bntx-formats.c (run at the top
  # of this script); the file-level checks below need synthetic fixtures
  # from tests/mk_bntx_extra.py built on a wimgt-encoded base container.
  command -v python3 >/dev/null || { sk "BNTX LegacySwitchLibraries ports"; return; }
  local d; d=$(mktemp -d /tmp/_r_bntx_legacy.XXXXXX) || { no "BNTX LegacySwitchLibraries ports" "mktemp failed"; return; }
  ...
  && python3 "$PWD_PROJECT/../tests/mk_bntx_extra.py" "$d/base.bntx" "$d/multi.bntx" "$d/ud.bntx" >/dev/null 2>&1 \
  || { no "BNTX LegacySwitchLibraries ports" "fixture build failed"; rm -rf "$d"; return; }
  # Multi-texture decode: both textures come out as .imgNNN.png with
  # pixel-exact colours, and no bare single-file output is written.
  "$B/wimgt" DECODE "$d/multi.bntx" --dest "$d/multi.png" --overwrite >/dev/null 2>&1
  if [ -s "$d/multi.img000.png" ] && [ -s "$d/multi.img001.png" ] && [ ! -f "$d/multi.png" ] \
  && python3 "$PNGTOOL" pixel "$d/multi.img000.png" 0 0 100 150 200 255 2>/dev/null \
  && python3 "$PNGTOOL" pixel "$d/multi.img000.png" 15 15 100 150 200 255 2>/dev/null \
  && python3 "$PNGTOOL" pixel "$d/multi.img001.png" 0 0 100 150 200 255 2>/dev/null \
  && python3 "$PNGTOOL" pixel "$d/multi.img001.png" 15 15 100 150 200 255 2>/dev/null; then
    ok "BNTX (Switch) -> PNG (2 textures, pixel-exact)"
  else
    no "BNTX (Switch) multi-texture" "decode mismatch"
  fi
  # Library-level multi-texture proof (both indices decode identically).
  if ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
      ../tests/test-bntx-multitex.c ./lib-bntx.o src/astc/astc_wrapper.o src/astc/astc_decomp.o \
      src/bcn-decoder/bcn_wrapper.o src/bcn-decoder/bcn.o -lc++ -Wl,--gc-sections \
      -o "$d/r_bntx_multitex" >/dev/null 2>&1 \
  || ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
      ../tests/test-bntx-multitex.c ./lib-bntx.o src/astc/astc_wrapper.o src/astc/astc_decomp.o \
      src/bcn-decoder/bcn_wrapper.o src/bcn-decoder/bcn.o -lc++ -Wl,-dead_strip \
      -o "$d/r_bntx_multitex" >/dev/null 2>&1; then
    if "$d/r_bntx_multitex" "$d/multi.bntx" >/dev/null 2>&1; then
      ok "BNTX multi-texture library decode (index 0 + 1)"
    else
      no "BNTX multi-texture library decode" "runtime check failed"
    fi
  else
    no "BNTX multi-texture library decode" "build failed"
  fi
  # UserData STRING/WSTRING string arrays + single-texture naming.
  if ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
      ../tests/test-bntx-userdata.c ./lib-bntx.o src/astc/astc_wrapper.o src/astc/astc_decomp.o \
      src/bcn-decoder/bcn_wrapper.o src/bcn-decoder/bcn.o -lc++ -Wl,--gc-sections \
      -o "$d/r_bntx_userdata" >/dev/null 2>&1 \
  || ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
      ../tests/test-bntx-userdata.c ./lib-bntx.o src/astc/astc_wrapper.o src/astc/astc_decomp.o \
      src/bcn-decoder/bcn_wrapper.o src/bcn-decoder/bcn.o -lc++ -Wl,-dead_strip \
      -o "$d/r_bntx_userdata" >/dev/null 2>&1; then
    if "$d/r_bntx_userdata" "$d/ud.bntx" >/dev/null 2>&1; then
      ok "BNTX UserData STRING/WSTRING string arrays"
    else
      no "BNTX UserData STRING/WSTRING" "runtime check failed"
    fi
  else
    no "BNTX UserData STRING/WSTRING" "build failed"
  fi
  "$B/wimgt" DECODE "$d/ud.bntx" --dest "$d/ud.png" --overwrite >/dev/null 2>&1
  if [ -s "$d/ud.png" ] && [ ! -f "$d/ud.img000.png" ] \
  && python3 "$PNGTOOL" pixel "$d/ud.png" 0 0 100 150 200 255 2>/dev/null \
  && python3 "$PNGTOOL" pixel "$d/ud.png" 15 15 100 150 200 255 2>/dev/null; then
    ok "BNTX UserData container still decodes (single-file naming)"
  else
    no "BNTX UserData container decode" "mismatch"
  fi
  # Switch BFRES archive census on the Tomodachi Life retail fixture
  # (1 FMDL, 6 FSKA, no FMAA/FBVS/FSHA/FSCN/EXTF).
  local bfres="$PWD_PROJECT/../tests/fixtures/bfres_switch_tomodachi_penguin.bfres"
  if [ -f "$bfres" ]; then
    if ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
        ../tests/test-bfres-switch-census.c ./lib-bfres.o -Wl,--gc-sections \
        -o "$d/r_bfres_census" >/dev/null 2>&1 \
    || ${CC:-cc} -O2 -ffunction-sections -fdata-sections -Isrc -Idclib \
        ../tests/test-bfres-switch-census.c ./lib-bfres.o -Wl,-dead_strip \
        -o "$d/r_bfres_census" >/dev/null 2>&1; then
      if "$d/r_bfres_census" "$bfres" >/dev/null 2>&1; then
        ok "BFRES Switch archive census (FMDL + 6 FSKA)"
      else
        no "BFRES Switch archive census" "runtime check failed"
      fi
    else
      no "BFRES Switch archive census" "build failed"
    fi
  else
    sk "BFRES Switch archive census"
  fi
  rm -rf "$d"
}
t_bntx_legacy
...
t_xtx(){
  ...
  # via the BNTX Tegra pipeline, format table from Switch-Toolbox XTX.cs).
  # No retail .xtx is on disk, so tests/mk_xtx.py builds a synthetic
  # two-texture file with an independent python swizzler (same GOB
  # equations BntxDeswizzle inverts): tex 0 is RGBA8 16x16 red/green
  # checkerboard, tex 1 is DXT1 16x16 solid red. Both must come out as
  # separate .imgNNN.png files with pixel-exact colours -- this also pins
  # the multi-texture container export (BNTX/GTX/XTX/NUT) that used to
  # silently drop every texture but index 0.
  rm -f /tmp/_r_xtx.xtx /tmp/_r_xtx.img000.png /tmp/_r_xtx.img001.png
  python3 "$PWD_PROJECT/../tests/mk_xtx.py" /tmp/_r_xtx.xtx >/tmp/_r_xtx_mk.log 2>&1 \
    || { no "XTX (Switch) synthetic fixture" "$(tail -1 /tmp/_r_xtx_mk.log 2>/dev/null)"; return; }
  $B/wimgt DECODE /tmp/_r_xtx.xtx --dest /tmp/_r_xtx.png --overwrite >/tmp/_r_xtx.log 2>&1
  if [ -s /tmp/_r_xtx.img000.png ] && [ -s /tmp/_r_xtx.img001.png ] \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img000.png 0 0 255 0 0 255 2>/dev/null \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img000.png 4 0 0 255 0 255 2>/dev/null \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img000.png 0 4 0 255 0 255 2>/dev/null \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img000.png 15 15 255 0 0 255 2>/dev/null \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img001.png 0 0 255 0 0 255 2>/dev/null \
    && python3 "$PNGTOOL" pixel /tmp/_r_xtx.img001.png 15 15 255 0 0 255 2>/dev/null; then
    ok "XTX (Switch) -> PNG (2 textures, pixel-exact RGBA8 + DXT1)"
  else
    no "XTX (Switch) -> PNG" "/tmp/_r_xtx.log"
  fi
  # Raw archive extraction is unchanged: two texture_*.bin members.
  rm -rf /tmp/_r_xtx.d; mkdir -p /tmp/_r_xtx.d
  $B/wszst EXTRACT /tmp/_r_xtx.xtx --dest /tmp/_r_xtx.d --overwrite >/tmp/_r_xtx_x.log 2>&1
  local n; n=$(find /tmp/_r_xtx.d -type f -size +0c 2>/dev/null | wc -l | tr -d ' ')
  [ "$n" = 2 ] && ok "XTX (Switch) EXTRACT -> 2 raw members" \
    || no "XTX (Switch) EXTRACT" "members=$n"
}
t_xtx
...
open('$d/huff8.bin', 'wb').write(bytes([0x28, 8, 0, 0, 1, 0xC0, ord(\"A\"), ord(\"B\")]) + stream)
open('$d/huff4.bin', 'wb').write(bytes([0x24, 4, 0, 0, 1, 0xC0, 1, 2]) + stream)
"
  local ok=1
...
' "$d/gfpak_test/sample.gfpak"
  ...
  if python3 "$PWD_PROJECT/../tests/mk_asobo.py" "$d/asobo_test" >/dev/null 2>&1; then
    ...
    && [ "$(head -c 4 "$d/asobo_test/out/0000def0.glb")" = "glTF" ] \
    && fok "Asobo BigFile (.DRV) unpacks stored and LZRS members; Bitmap_Z -> PNG, Sound_Z -> WAV, Mesh_Z -> GLB" \
  ...
  if python3 "$PWD_PROJECT/../tests/mk_bombshell.py" "$d/bombshell_test" >/dev/null 2>&1; then
    ...
    && [ "$(head -c 4 "$d/bombshell_test/T.xwi.d/0_world/models/tri_model.glb")" = "glTF" ] \
...
' "$g1t_le" "$d/g1t_be.g1t" 2>/dev/null
  ...
  # BNSH BinaryShaderLibrary coverage: dual source/binary programs per variation,
  # a zlib-compressed stage and per-program MemoryData, plus .glsl/.zlib.bin sidecars.
  python3 -c '
import struct
d = bytearray(2048)
def w32(o, v): struct.pack_into("<I", d, o, v)
def w16(o, v): struct.pack_into("<H", d, o, v)
def w64(o, v): struct.pack_into("<Q", d, o, v)
d[0:4] = b"BNSH"
w32(8, 0x00020001); w16(12, 0xFEFF); d[14] = 2; d[15] = 8
w32(28, 2048)
d[0x60:0x64] = b"grsc"; w32(0x64, 96); w64(0x68, 256)
w32(0x7c, 1); w32(0x80, 0x90)
w64(0x90, 0x200); w64(0xA0, 0x400); w64(0xA8, 0x60)
d[0x200] = 1; d[0x201] = 3; w32(0x204, 0); w64(0x208, 0x300)
struct.pack_into("<H", d, 0x300, 1); w64(0x308, 0x340); w64(0x310, 0x350)
w32(0x340, 13); w64(0x350, 0x360); d[0x360:0x36D] = b"void main(){}"
d[0x400] = 2; d[0x401] = 0; w32(0x404, 1); w64(0x408, 0x500)
w32(0x500, 8); w32(0x504, 16); w64(0x508, 0x520)
d[0x520:0x528] = b"\x78\x9c\x01\x02\x03\x04\x05\x06"
w32(0x460, 4); w64(0x468, 0x530)
open("'"$d"'/bnsh_test/dual.bnsh", "wb").write(bytes(d))
' 2>/dev/null
  if "$B/wszst" EXTRACT "$d/bnsh_test/dual.bnsh" --dest "$d/bnsh_test/dual.bnsh.txt" --overwrite >/dev/null 2>&1 \
  && grep -q "source_program: shader_type = 1" "$d/bnsh_test/dual.bnsh.txt" \
  && grep -q "binary_program: shader_type = 2" "$d/bnsh_test/dual.bnsh.txt" \
  && grep -q "compression = 1 (zlib)" "$d/bnsh_test/dual.bnsh.txt" \
  && grep -q "memory: offset = " "$d/bnsh_test/dual.bnsh.txt" \
  && [ -s "$d/bnsh_test/dual.v00.source.vertex.0.glsl" ] \
  && [ -s "$d/bnsh_test/dual.v00.binary.vertex.0.zlib.bin" ]; then
    fok "BNSH dual programs + compressed stage + MemoryData (+ sidecars)"
  else
    fno "BNSH BinaryShaderLibrary" "dual/compressed/memory decode or sidecars failed";
  fi

  # BFSHA BinaryShaderLibrary coverage: 40-byte option records with choice values,
  # sampler Extra strings, Material block-type names, embedded-.bnsh sidecar.
  python3 -c '
import struct
def u16s(s): return struct.pack("<H", len(s)) + s.encode() + b"\x00"
buf = bytearray(4096)
def w64(o, v): struct.pack_into("<Q", buf, o, v)
def w32(o, v): struct.pack_into("<I", buf, o, v)
def w16(o, v): struct.pack_into("<H", buf, o, v)
buf[0:4] = b"FSHA"; buf[4:8] = b"    "
buf[8] = 0; buf[9] = 1; struct.pack_into("<H", buf, 10, 4)
struct.pack_into("<H", buf, 12, 0xFEFF); buf[14] = 2; buf[15] = 8
w32(0x18, 0)
p = 0x20
p += 24
o_name = p; p += 8; o_path = p; p += 8; o_arr = p; p += 8; o_dict = p; p += 8
for _ in range(4): w32(p, 0); p += 4
w64(p, 0); p += 8
w16(p, 1); p += 2; w16(p, 0); p += 2; w16(p, 0); p += 2
p = 0x80
def alloc_string(s):
    global p
    off = p; b = u16s(s); buf[p:p+len(b)] = b; p += len(b)
    while p % 4: p += 1
    return off
s_name = alloc_string("TestArch"); s_path = alloc_string("content:/t.bfsha")
s_model = alloc_string("M1"); s_opt = alloc_string("OPT1")
s_chA = alloc_string("A"); s_chB = alloc_string("B")
s_smp = alloc_string("SMP1"); s_extra = alloc_string("anno1")
s_blk = alloc_string("BLK1"); s_uni = alloc_string("U1")
def alloc_dict(keys):
    global p
    off = p
    w32(p, 0); p += 4; w32(p, len(keys)); p += 4
    w32(p, 0xFFFFFFFF); p += 4; w16(p, 0); p += 2; w16(p, 0); p += 2; w64(p, 0); p += 8
    for k in keys:
        w32(p, 0); p += 4; w16(p, 0); p += 2; w16(p, 0); p += 2; w64(p, k); p += 8
    return off
d_model = alloc_dict([s_model]); d_opt = alloc_dict([s_opt])
d_ch = alloc_dict([s_chA, s_chB]); d_smp = alloc_dict([s_smp])
d_blk = alloc_dict([s_blk]); d_uni = alloc_dict([s_uni])
a_opt = p; p += 40
w64(a_opt, s_opt); w64(a_opt+8, d_ch)
o_choicevals = p; w64(a_opt+16, o_choicevals)
buf[a_opt+24] = 2; buf[a_opt+25] = 0; buf[a_opt+26] = 0
struct.pack_into("<H", buf, a_opt+27, 0)
buf[a_opt+29] = 0; buf[a_opt+30] = 0; buf[a_opt+31] = 0
w32(a_opt+32, 0xF); w32(a_opt+36, 0)
w32(p, 10); w32(p+4, 20); p += 8
a_smp = p; p += 16
w64(a_smp, s_extra); buf[a_smp+8] = 3
a_blk = p; p += 32
o_uniarr = p
w64(a_blk, o_uniarr); w64(a_blk+8, d_uni); w64(a_blk+16, 0)
buf[a_blk+24] = 0; buf[a_blk+25] = 1
w16(a_blk+26, 16); w16(a_blk+28, 1); w16(a_blk+30, 0)
w64(p, s_uni); w32(p+8, 5); w16(p+12, 0); buf[p+14] = 0; p += 16
a_model = p; p += 192
mp = a_model
w64(mp, s_model); mp += 8
w64(mp, a_opt); w64(mp+8, d_opt); mp += 16
w64(mp, 0); w64(mp+8, 0); mp += 16
w64(mp, 0); w64(mp+8, 0); mp += 16
w64(mp, a_smp); w64(mp+8, d_smp); mp += 16
w64(mp, a_blk); w64(mp+8, d_blk); mp += 16
w64(mp, o_uniarr); mp += 8
o_prog = p; p += 0x30
o_keys = p; w32(p, 7); p += 4
o_bnsh = p
buf[p:p+4] = b"BNSH"; w32(p+0x1C, 64); p += 64
w64(mp, o_prog); mp += 8; w64(mp, o_keys); mp += 8
w64(mp, 0); mp += 8; w64(mp, 0); mp += 8; w64(mp, o_bnsh); mp += 8
mp += 24
w32(mp, 1); mp += 4; w32(mp, 0); mp += 4
w16(mp, 1); mp += 2; w16(mp, 0); mp += 2; w16(mp, 1); mp += 2; w16(mp, 0); mp += 2
buf[mp] = 1; mp += 1; buf[mp] = 0; mp += 1
buf[mp] = 0; mp += 1; buf[mp] = 1; mp += 1
buf[mp] = 1; mp += 1; buf[mp] = 0; mp += 1
mp += 4; mp += 6
assert mp == a_model + 192
w64(o_name, s_name); w64(o_path, s_path)
w64(o_arr, a_model); w64(o_dict, d_model)
w32(0x1C, p)
open("'"$d"'/bnsh_test/mini.bfsha", "wb").write(bytes(buf[:p]))
' 2>/dev/null
  if "$B/wszst" EXTRACT "$d/bnsh_test/mini.bfsha" --dest "$d/bnsh_test/mini.bfsha.txt" --overwrite >/dev/null 2>&1 \
  && grep -q "choice_values = 10 20" "$d/bnsh_test/mini.bfsha.txt" \
  && grep -q "extra=anno1" "$d/bnsh_test/mini.bfsha.txt" \
  && grep -q "type=Material(1)" "$d/bnsh_test/mini.bfsha.txt" \
  && [ -s "$d/bnsh_test/mini.M1.bnsh" ]; then
    fok "BFSHA option values + sampler extras + block types (+ embedded .bnsh)"
  else
    fno "BFSHA BinaryShaderLibrary" "option/sampler/block decode or sidecar failed";
  fi
...
# Smash XMB (.xmb) material/LOD metadata. The synthetic file mirrors
# XMBDec.py's layout (header, node table, property table, string tables);
# decode must reproduce XMBDec's XML mapping byte for byte.
t_xmb_decode(){
  local d; d=$(mktemp -d /tmp/_r_xmb.XXXXXX) || { no "XMB decode" "mktemp failed"; return; }
  python3 "$PWD_PROJECT/../tests/mk_xmb.py" "$d/model.xmb" >/dev/null 2>&1
  if "$B/wszst" FILETYPE "$d/model.xmb" 2>/dev/null | grep -q "^XMB"; then
    ok "XMB recognised by FILETYPE"
  else
    no "XMB filetype" "FILETYPE did not report XMB"
  fi
  if "$B/wszst" EXTRACT "$d/model.xmb" --overwrite >/dev/null 2>&1 \
  && python3 -c '
import sys, xml.etree.ElementTree as ET
raw = open(sys.argv[1], "rb").read()
assert raw == b"""<?xml version=\"1.0\" ?>
<model type=\"effect_main\">
    <draw buffer=\"0\"/>
</model>
""", "XML bytes differ:\n" + raw.decode()
root = ET.fromstring(raw.split(b"\n", 1)[1])
assert root.tag == "model" and root.get("type") == "effect_main"
(kid,) = list(root)
assert kid.tag == "draw" and kid.get("buffer") == "0"
' "$d/model.xmb.xml"; then
    ok "XMB -> XMBDec-mapping XML"
  else
    no "XMB decode" "XML missing or wrong contents"
  fi

  # A truncated node table must be refused, not read anyway.
  python3 -c '
import sys
d = bytearray(open(sys.argv[1], "rb").read())
open(sys.argv[1], "wb").write(bytes(d[:60]))
' "$d/model.xmb"
  rm -f "$d/model.xmb.xml"
  "$B/wszst" EXTRACT "$d/model.xmb" >/dev/null 2>&1
  if [ ! -s "$d/model.xmb.xml" ]; then
    ok "XMB refuses a truncated node table"
  else
    no "XMB decode" "decoded a truncated file"
  fi
  rm -rf "$d"
}
t_xmb_decode

# Smash mesh triangle adjacency (model.adjb). Sizing follows Adjb.cs:
# each mesh runs to the next higher offset, the last to end of file.
t_adjb_decode(){
  local d; d=$(mktemp -d /tmp/_r_adjb.XXXXXX) || { no "ADJB decode" "mktemp failed"; return; }
  python3 "$PWD_PROJECT/../tests/mk_adjb.py" "$d/model.adjb" >/dev/null 2>&1
  if "$B/wszst" FILETYPE "$d/model.adjb" 2>/dev/null | grep -q "^ADJB"; then
    ok "ADJB recognised by FILETYPE"
  else
    no "ADJB filetype" "FILETYPE did not report ADJB"
  fi
  if "$B/wszst" EXTRACT "$d/model.adjb" --overwrite >/dev/null 2>&1 \
  && python3 -c '
import sys
...
[meshes]
  [0] id = 3, index_count = 3
    10 20 30
  [1] id = 7, index_count = 2
    40 50
""", "manifest differs:\n" + raw
' "$d/model.adjb.txt"; then
    ok "ADJB -> index manifest"
  else
    no "ADJB decode" "manifest missing or wrong contents"
  fi
  rm -rf "$d"
}
t_adjb_decode

# SSBH MATL (.numatb) MatLab-dialect XML export beside the text manifest.
t_numatb_xml(){
  local d; d=$(mktemp -d /tmp/_r_numatb.XXXXXX) || { no "NUMATB XML" "mktemp failed"; return; }
  python3 "$PWD_PROJECT/../tests/mk_numatb.py" "$d/m.numatb" >/dev/null 2>&1
  if "$B/wszst" EXTRACT "$d/m.numatb" --overwrite >/dev/null 2>&1 \
  && [ -s "$d/m.numatb.txt" ] \
  && python3 -c '
import sys, xml.etree.ElementTree as ET
root = ET.parse(sys.argv[1]).getroot()
assert root.tag == "MaterialLibrary", root.tag
(mat,) = list(root)
assert mat.get("shaderLabel") == "shd", mat.get("shaderLabel")
assert mat.get("materialLabel") == "mat0", mat.get("materialLabel")
got = {p.get("name"): p[0] for p in mat}
assert got["CustomFloat0"].tag == "Float" and got["CustomFloat0"].text == "1.5"
assert got["CustomBoolean1"].tag == "Bool" and got["CustomBoolean1"].text == "true"
v4 = got["CustomVector0"]
assert v4.tag == "Vector4"
assert [c.text for c in v4] == ["1", "0.5", "0", "1"], [c.text for c in v4]
st = got["Texture0"]
assert st.tag == "String" and st[0].tag == "Text" and st[0].text == "tex_col"
sa = got["Sampler0"]
fields = {c.tag: (c.text if len(c) == 0 else c) for c in sa}
assert fields["WrapS"] == "Repeat", fields
assert fields["MinFilter"] == "LinearMipmapLinear", fields
assert fields["MaxAnisotropy"] == "1", fields
bc = fields["BorderColor"]
assert [c.text for c in bc] == ["0", "0", "0", "0"]
' "$d/m.numatb.xml"; then
    ok "NUMATB -> MatLab-dialect XML plus text manifest"
  else
    no "NUMATB XML" "XML missing or wrong contents"
  fi
  rm -rf "$d"
}
t_numatb_xml
...
t_prc_paracobn(){
  local d; d=$(mktemp -d /tmp/_r_prc.XXXXXX) || { no "PRC decode" "mktemp failed"; return; }
  python3 "$PWD_PROJECT/../tests/mk_prc.py" "$d/test.prc" >/dev/null 2>&1
  if "$B/wszst" EXTRACT "$d/test.prc" --overwrite >/dev/null 2>&1 \
  && python3 -c '
import sys, xml.etree.ElementTree as ET
root = ET.parse(sys.argv[1]).getroot()
assert root.tag == "struct" and root.get("hash") is None
kids = list(root)
assert [k.get("hash") for k in kids] == ["0x1111111111", "0x2222222222"], "keys unsorted"
b, lst = kids
assert b.tag == "bool" and b.text == "true"
items = list(lst)
assert lst.tag == "list" and [i.get("index") for i in items] == ["0", "1", "2"]
assert [(i.tag, i.text) for i in items] == [("sbyte", "-5"), ("float", "0.5"), ("string", "hi")]
' "$d/test.prc.xml"; then
    ok "PRC paracobn -> ParamXML-dialect XML"
  else
    no "PRC decode" "XML missing or wrong contents"
  fi

  # A bad root type must be refused, not misread.
  python3 -c '
import struct, sys
d = bytearray(open(sys.argv[1], "rb").read())
struct.pack_into("<B", d, 16 + 16 + 19, 1)
open(sys.argv[1], "wb").write(d)
' "$d/test.prc"
  rm -f "$d/test.prc.xml"
  "$B/wszst" EXTRACT "$d/test.prc" >/dev/null 2>&1
  if [ ! -s "$d/test.prc.xml" ]; then
    ok "PRC refuses a non-struct root"
  else
    no "PRC decode" "decoded a bad root type"
  fi
  rm -rf "$d"
}
t_prc_paracobn

# SSBH ANIM (.nuanmb) keyframe payloads: Direct floats, a Constant
# transform, Compressed booleans and a Compressed transform whose X channel
# lerps 0 -> 4 while W is reconstructed.
t_nuanmb_payload(){
  local d; d=$(mktemp -d /tmp/_r_nuanmb.XXXXXX) || { no "NUANMB payload" "mktemp failed"; return; }
  python3 "$PWD_PROJECT/../tests/mk_nuanmb.py" "$d/a.nuanmb" >/dev/null 2>&1
  if "$B/wszst" EXTRACT "$d/a.nuanmb" --overwrite >/dev/null 2>&1 \
  && python3 -c '
import sys
raw = open(sys.argv[1]).read()
for want in [
  "[0] 1.5",
  "[1] 2.5",
  "[0] pos=(1, 2, 3) rot=(0, 0, 0, 1) scale=(1, 1, 1) compensate=0",
  "[0] true",
  "[1] false",
  "[2] true",
  "[0] pos=(0, 0, 0) rot=(0, 0, 0, 1) scale=(1, 1, 1) compensate=0",
  "[1] pos=(4, 0, 0) rot=(0, 0, 0, 1) scale=(1, 1, 1) compensate=0",
]:
    assert want in raw, "missing payload line: " + want
' "$d/a.nuanmb.txt"; then
    ok "NUANMB keyframe payloads decode"
  else
    no "NUANMB payload" "payload lines missing or wrong"
  fi
  rm -rf "$d"
}
t_nuanmb_payload
