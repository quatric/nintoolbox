...
enumError ScanNitroNFTR (nitro_nftr_t *nftr, const u8 *data, uint size)
{
	...
	if (cglp)
	{
		...
		const uint bytes_per_glyph
			= cell_sz ? cell_sz : (nftr->cell_w * nftr->cell_h * nftr->bpp + 7) / 8;
		...
		for (uint py = 0; py < nftr.cell_h; py++)
			for (uint px = 0; px < nftr.cell_w; px++)
			{
				...
				else if (nftr.bpp == 3)
				{
					...
					uint boff = bitpos & 7;
					uint raw_val = (gsrc[bstart] << 8)
						| (bstart + 1 < bytes_per_glyph ? gsrc[bstart + 1] : 0);
...
enumError create_nanr_xml (ccp source, ccp dest)
{
	...
	mxml_node_t *anim_node
		= mxmlFindElement (nanr_node, nanr_node, "animation", NULL, NULL, MXML_DESCEND_FIRST);
	for (uint i = 0; !err && i < n_anims; i++)
	{
		...
		if (index != i || !count || count > n_frames - frame_used)
		{
			err = ERR_INVALID_DATA;
			break;
		}
		...
		mxml_node_t *frame_node
			= mxmlFindElement (anim_node, anim_node, "frame", NULL, NULL, MXML_DESCEND_FIRST);
		for (uint j = 0; j < count; j++)
		{
			...
			if (!cell_attr || !dur_attr || !off_attr)
			{
				err = ERR_INVALID_DATA;
				break;
			}
			...
			if (cell > 0xffff || duration > 0xffff || off > 0xfffffffdu || off + 2 < off)
			{
				err = ERR_INVALID_DATA;
				break;
			}
			...
			frame_node
				= mxmlFindElement (frame_node, anim_node, "frame", NULL, NULL, MXML_NO_DESCEND);
		}
		anim_node
			= mxmlFindElement (anim_node, nanr_node, "animation", NULL, NULL, MXML_NO_DESCEND);
	}
	mxmlDelete (tree);
