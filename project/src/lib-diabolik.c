...
#define DBK_MAX_NAME_LEN  64   // ASCII name/id fields are 20 bytes in every
                                // sample seen; allow generous headroom
#define DBK_MAX_TEXT_CHARS 8192 // UTF-16BE dialogue lines; longest real
                                 // sample seen is well under this
#define DBK_BLOB_MIN      4096  // LEAF payloads at/above this size are
...
static u32 try_ascii_field (const u8 *data, size_t size, size_t off, const char **text, u32 *text_len)
{
	if (off + 4 > size)
		return 0;
...
static int utf8_put (char *out, size_t out_room, u16 cp)
{
	if (cp < 0x80)
	{
		if (out_room < 1) return 0;
		out[0] = (char) cp;
		return 1;
	}
	if (cp < 0x800)
	{
		if (out_room < 2) return 0;
		out[0] = (char) (0xc0 | (cp >> 6));
		out[1] = (char) (0x80 | (cp & 0x3f));
		return 2;
	}
	if (out_room < 3) return 0;
...
static u32 try_utf16_field (const u8 *data, size_t size, size_t off,
                             char *utf8_out, size_t utf8_room)
{
	if (off + 4 > size)
		return 0;
...
static void walk_block (FILE *f, const u8 *data, size_t off, size_t end, int depth,
                         u32 *n_blocks, u32 *n_blobs)
{
	...
	while (pos + 8 <= end)
	{
		...
		fprintf (f, "%*s%-7s @0x%06zx size=%-8u payload=0x%06zx..0x%06zx (%u bytes)\n",
			depth * 2, "", tag == DBK_TAG_SECTION ? "SECTION" : "LEAF",
			pos, blk_size, payload_off, payload_end, payload_len);
		(*n_blocks)++;
