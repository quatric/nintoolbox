...
enum
{
	OCTO_SEC_HEADER_SIZE = 0x20,
	OCTO_SEC_ENTRY_SIZE = 0x20
};
...
enumError DecodeOctomaniaSec_Text (FILE *f, const u8 *data, size_t size, size_t file_size)
{
	...
	fprintf (
		f, "block_size = 0x%x  # big-endian, constant 0x800 in every sample seen\n", block_size);
	...
	for (u32 i = 0; i < count; i++)
	{
		...
		fprintf (f,
			"chunk[%u]: field0=%u index=%u tag=%u name_ofs_plus1=%u"
