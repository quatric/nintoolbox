...
static u32 vb_rd32 (const u8 *p) { return p[0] | p[1] << 8 | p[2] << 16 | (u32)p[3] << 24; }
static u32 vb_rd32be (const u8 *p) { return (u32)p[0] << 24 | p[1] << 16 | p[2] << 8 | p[3]; }
...
{
	if (n >= 4 && !memcmp (d, "\xff" "REC", 4)) return ".rec";
	if (n >= 4 && !memcmp (d, "\xff" "MAP", 4)) return ".map";
	if (n >= 4 && !memcmp (d, "\xff" "TIL", 4)) return ".til";
	if (n >= 4 && !memcmp (d, "\xff" "BMD", 4)) return ".bmd";
	if (n >= 4 && !memcmp (d, "\xff" "AN2", 4)) return ".an2";
	if (n >= 4 && !memcmp (d, "\xff" "PFX", 4)) return ".pfx";
	if (n >= 4 && !memcmp (d, "\xff" "FRC", 4)) return ".frc";
	if (n >= 4 && !memcmp (d, "BTRK", 4)) return ".btrk";
	if (n >= 4 && !memcmp (d, "OggS", 4)) return ".ogg";
	if (n >= 4 && !memcmp (d, "RIFF", 4)) return ".wav";
	return ".bin";
}

// Name of the member ('name.ext' if known, else 'hash.ext'); ext comes from the content.
static void vb_member_name (char *dest, size_t sz, u32 hash, uint idx, bool slot, const u8 *d, uint n)
{
...
static bool vb_add_member (nintendo_sarc_entry_t *out, uint *n, const u8 *data, size_t total, u32 hash,
	uint idx, bool slot, u32 off, u32 size, u32 stored)
{
...
enumError ScanVblankBfp (nintendo_sarc_entry_t **entries, uint *n_entries, const u8 *data, size_t size)
{
	...
	*n_entries = 0;
	...
	for (uint i = 0; i < cnt; i++)
	{
		...
		if (!vb_add_member (out, &n, data, size, vb_rd32 (e), i, false, vb_rd32 (e + 4), vb_rd32 (e + 8),
				vb_rd32 (e + 12)))
			goto fail;
	}
	for (uint i = 0; i < 256; i++)
	{
		...
		if (!vb_rd32 (e) && !vb_rd32 (e + 4) && !vb_rd32 (e + 8))
			continue;
		if (!vb_add_member (out, &n, data, size, 0, i, true, vb_rd32 (e), vb_rd32 (e + 4), vb_rd32 (e + 8)))
			goto fail;
	}
	if (!n)
		goto fail;
...
bool IsVblankBap (const u8 *data, size_t size)
{
	...
	{
		const u32 base = vb_rd32 (data + 4), tab = vb_rd32 (data + 0x14), n = data[0x10] | data[0x11] << 8;
...
static enumError vb_scan_bpp3 (nintendo_sarc_entry_t **entries, uint *n_entries, const u8 *data, size_t size)
{
	const u32 base = vb_rd32 (data + 4), tab = vb_rd32 (data + 0x14), cnt = data[0x10] | data[0x11] << 8;
	...
	for (uint i = 0; i < cnt; i++)
	{
		...
		for (uint s = 0; s < ns && n < cap; s++)
		{
			...
			if ((u64)sd + 24 > size)
				goto fail;
			const u32 kind = vb_rd32 (data + sd), sz = vb_rd32 (data + sd + 4), off = vb_rd32 (data + sd + 20);
			...
			const bool ok = wav ? OwnedEntryAdd (out, n, name, wav, (uint)wsz) : OwnedEntryAdd (out, n, name, d, sz);
			FREE (wav);
			if (!ok)
				goto fail;
			n++;
...
static enumError vb_scan_bap1 (nintendo_sarc_entry_t **entries, uint *n_entries, const u8 *data, size_t size)
{
	...
	if (pool && pool < size && OwnedEntryAdd (out, n, "samples.raw", data + pool, (uint)(size - pool)))
		n++;
	if (!n)
		goto fail;
	*entries = out;
