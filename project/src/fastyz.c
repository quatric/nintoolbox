...
#include <intrin.h>
static uint32_t FASTYZ_CTZ32 (uint32_t x)
{
	unsigned long i;
	_BitScanForward (&i, x);
	return (uint32_t)i;
}
static uint32_t FASTYZ_CLZ32 (uint32_t x)
{
	unsigned long i;
	_BitScanReverse (&i, x);
...
static uint32_t FASTYZ_CTZ64 (uint64_t x)
{
	unsigned long i;
	_BitScanForward64 (&i, x);
	return (uint32_t)i;
}
static uint32_t FASTYZ_CLZ64 (uint64_t x)
{
	unsigned long i;
...
static inline void writer_emit_match (yaz0_writer_t *w, uint32_t len, uint32_t distance)
{
	...
	if (FASTYZ_UNLIKELY (len > MAX_LEN))
	{
		while (len > MAX_LEN)
		{
			 ...
			 */
			uint32_t chunk = FASTYZ_UNLIKELY (len - MAX_LEN < SHORT_FORM_MIN)
				? (MAX_LEN - (SHORT_FORM_MIN - 1))
				: MAX_LEN;
...
static FASTYZ_ALWAYS_INLINE int compress_core (const void *input, int length, void *output,
#if FASTYZ_HTAB == FASTYZ_HTAB_SCRATCH
	void *scratch,
#endif
	const uint32_t hlog)
{
	...
	while (FASTYZ_LIKELY (ip < ip_limit))
	{
		...
		{
			seq = read_u32_le (ip)
				& 0xffffff; /* Use 3 bytes for hashing, the minimum match length */
			...
			cmp = FASTYZ_LIKELY (distance <= MAX_MATCH_DISTANCE) ? read_u32_le (ref) & 0xffffff
																 : 0x1000000;
		...
		uint32_t len
			= compare_match (ref + SHORT_FORM_MIN, ip + SHORT_FORM_MIN, ip_bound) + SHORT_FORM_MIN;
...
int yaz0_compress (const void *input, int length, void *output)
{
	...
	if (FASTYZ_HASH_LOG > 10 && length < (1 << 12))
		return compress_core (input, length, output,
#if FASTYZ_HTAB == FASTYZ_HTAB_SCRATCH
			scratch,
#endif
			10);
		...
		if (FASTYZ_LIKELY (dst < dst_fast && src < src_fast))
		{
			...
			for (int i = 0; i < YAZ0_FLAG_BYTE_NUM_BITS; ++i)
			{
				...
				else
				{
					...
					/* Validate back-reference and copy from it (byte-by-byte for overlapping
					 * copies) */
					...
					{
						/* overlapping run:
						   materialize 8 bytes, then move the source so the remainder has an
						   effective distance of at least 8 (standard offset-table trick) */
...
uint32_t yaz0_get_decompressed_size (const void *input)
{
	...
	const uint8_t *src = (const uint8_t *)input;
	return ((uint32_t)src[4] << 24) | ((uint32_t)src[5] << 16) | ((uint32_t)src[6] << 8)
		| ((uint32_t)src[7]);
}
