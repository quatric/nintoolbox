...
static void *CxiBitWriterGetBytes (
	CxiBitWriter *stream, int wordAlign, int beBytes, int beBits, unsigned int *size)
{
	// allocate buffer
	...
	if (!wordAlign)
	{
		...
		if (stream->nBitsInLastWord <= 24)
			outSize--;
		if (stream->nBitsInLastWord <= 16)
			outSize--;
		if (stream->nBitsInLastWord <= 8)
			outSize--;
		if (stream->nBitsInLastWord <= 0)
			outSize--;
	}
...
static void CxiBitWriterWriteBitsBE (CxiBitWriter *stream, uint32_t bits, unsigned int nBits)
{
	for (unsigned int i = 0; i < nBits; i++)
		CxiBitWriterWrite (stream, (bits >> (nBits - 1 - i)) & 1);
}
...
static void CxiLzStateInit (CxiLzState *state, // the LZ search state structure
...
//
static unsigned int CxiCompareMemory (const unsigned char *b1, // the first location
...
//
static int CxiLzConfirmMatch (const unsigned char *buffer, // the buffer
...
//
static unsigned int CxiLzSearch (CxiLzState *state, // the LZ search state
...
static unsigned int CxiLzSearchRestricted (
	CxiLzState *state, const unsigned int *pDstDepths, unsigned int *pDistance)
{
	unsigned int nBytesLeft = state->size - state->pos;
	if (nBytesLeft < 3 || nBytesLeft < state->minLength)
	{
...
static CxiLzToken *CxiLzTokenBufferAlloc (CxiLzTokenBuffer *buf)
{
	...
	if (buf->nTokens > buf->capacity)
	{
		...
		CxiLzToken *newbuf
			= (CxiLzToken *)REALLOC (buf->tokens, buf->capacity * sizeof (CxiLzToken));
		if (newbuf == NULL)
		{
			// alloc fail
			buf->capacity = cap1;
...
static int CxiHuffmanCountSymbolsOver (CxiHuffNode *tree, unsigned int nMin)
{
	...
	return CxiHuffmanCountSymbolsOver (tree->left, nMin)
		+ CxiHuffmanCountSymbolsOver (tree->right, nMin);
}
...
static CxiHuffSymbolInfo *CxiHuffmanEnumerateSymbolInfoInternal (
	CxiHuffNode *tree, CxiHuffSymbolInfo *buf, int depth, unsigned int nMin)
{
	if (tree->left == NULL)
	{
		if (tree->sym >= nMin)
...
static CxiHuffSymbolInfo *CxiHuffmanEnumerateSymbolInfo (
	CxiHuffNode *tree, unsigned int *pCount, unsigned int nMin)
{
...
//
static void CxiAshEnsureTreeElements (CxiHuffNode *nodes, // the Huffman code buffer
...
//    sub tree node right
//
static void CxiAshWriteTree (CxiBitWriter *stream, // the output bit stream to write the tree to
...
//
static CxiLzToken *CxiAshTokenizeGreedy (const unsigned char *buffer, // the input buffer
...
)
{
	...
	if (!CxiLzTokenBufferInit (&tokenBuffer))
		return NULL;
...
//
static void CxiAshGenHuffman (const CxiLzToken *tokens, // the input LZ tokenization
...
//
static unsigned int CxiAshRoundDown (unsigned int sym, // the input symbol value
...
//
static CxiLzToken *CxiAshRetokenize (const unsigned char *buffer, // the input buffer
	unsigned int size, // the size of the input buffer
	unsigned int nSymBits, // the symbol code width
	unsigned int nDstBits, // the distance code width
...
)
{
	...
	CxiHuffSymbolInfo *lenInfo
		= CxiHuffmanEnumerateSymbolInfo (symNodes, &nLenNodesAvailable, 0x100);
	...
	{
		if (CxiHuffmanHasSymbol (symNodes, i))
			symDepths[i] = CxiHuffmanGetNodeDepth (symNodes, i);
	}
	for (unsigned int i = 0; i < (1u << nDstBits); i++)
	{
		if (CxiHuffmanHasSymbol (dstNodes, i))
			dstDepths[i] = CxiHuffmanGetNodeDepth (dstNodes, i);
	}
	...
	for (unsigned int i = 0; i < nLenNodesAvailable; i++)
		lens[i] = lenInfo[i].sym - 0x100 + 3;
	for (unsigned int i = 0; i < nDstNodesAvailable; i++)
		dsts[i] = dstInfo[i].sym + 1;

	// get minimum distance node cost
	unsigned int minDstCost = UINT_MAX;
	for (unsigned int i = 0; i < (1u << nDstBits); i++)
	{
		if (dstDepths[i] && dstDepths[i] < minDstCost)
			minDstCost = dstDepths[i];
	}
	if (minDstCost == UINT_MAX)
		minDstCost = 0; // no distance nodes => min cost=0
	...
	if (size >= 3)
	{
		for (unsigned int i = 0; i < (size - 2); i++)
		{
			...
			hashbuf[i] = (buffer[i + 0] << 0) | (buffer[i + 1] << 8) | (buffer[i + 2] << 16);
		}
	}
	...
	while (pos-- > 0)
	{
		...
		// NOTE: all byte values that appear in the file will have a symbol associated since they
		// must appear at least once. thus we do not need to check that any byte value exists.
		...
		else
		{
			...
			CX_ASSERT (lengthIndex >= 0
				&& ((unsigned int)lengthIndex) < nLenNodesAvailable); // legnth index must be valid
			...
			while (length)
			{
				...
				// check if this node has better total weight. We allow the match to be equal since
				// we're scanning sizes down, we'll prefer shorter lengths. This pushes the
				// distribution of lengths to the lower end allowing for better Huffman coding. This
				// also emphasizes byte literals, which have no distance cost. We add the minimum
				// distance cost here to offset this (lack of) cost.
				if (thisWeight <= weightBest
					|| (length == 1 && thisWeight <= (weightBest + minDstCost)))
				{
				...
				lengthIndex--;
				if (lengthIndex >= 0)
					length = lens[lengthIndex]; // length in list
				else if (lengthIndex == -1)
					length = 1; // decrement to 1
				else
					break; // end
			}
			...
			{
				// we ended up selecting an LZ copy-able length. but did we select the most optimal
				// distance encoding? search possible distances where we can match the string at.
				// We'll take the lowest-cost one.
				for (unsigned int i = 0; i < nDstNodesAvailable; i++)
				{
					...
					if (hashbuf[pos] != hashbuf[pos - dst])
						continue;
...
//
static void CxiAshCreateTokens (CxiAshTokenization *pTok, // structure receiving the tokenization
...
static void CxiAshTokFree (CxiAshTokenization *pTok)
{
	...
	if (pTok->tokens != NULL)
		FREE (pTok->tokens);
	if (pTok->symTree != NULL)
		FREE (pTok->symTree);
	if (pTok->dstTree != NULL)
		FREE (pTok->dstTree);
...
// code sizes.
//
unsigned char *CxCompressAsh (const unsigned char *buffer, // the input data
