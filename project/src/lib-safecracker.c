...
enumError DecodeSafecrackerTOC (sf_toc_t *toc, const u8 *data, size_t size)
{
	...
	for (u32 i = 0; i < toc->slot_count && off + SF_TOC_RECORD_SIZE <= size;
		i++, off += SF_TOC_RECORD_SIZE)
	{
