...
enumError GetCMABEntry (const cmab_t *cmab, uint index, cmab_entry_t *entry)
{
	...
	if (!data_size || data_offset > cmab->size || data_size > cmab->size - data_offset
		|| name_start >= cmab->size)
		return EINVAL;
	...
	const size_t name_len
		= nul - name < sizeof (entry->name) - 1 ? nul - name : sizeof (entry->name) - 1;
...
enumError DecodeCMABTexture_RGBA (u8 **dest, uint *width, uint *height, const cmab_entry_t *entry)
{
	...
	return DecodePicaTexture (dest, width, height, entry->data, entry->width, entry->height,
		entry->pica_format, entry->data_size);
}
