...
static bool smashacmd_ok (const u8 *data, size_t size,
	u32 *actions, bool *be)
{
	...
	for (u32 i = 0; i < n; i++)
	{
		...
		const u32 next = i + 1 < n
			? smashacmd_rd32 (data + 16 + (size_t)(i + 1) * 8 + 4, !le)
			: (u32)size;
		...
		if (next > off
			&& smashacmd_rd32 (data + next - 4, !le) != SMASH_ACMD_END)
			return false;
		prev = off;
	}
	*actions = n;
...
enumError DecodeSmashACMD_Text (FILE *out, const u8 *data, size_t size)
{
	...
	fprintf (out, "#ACMD\n# Super Smash Bros. 4 moveset script file\n\n"
		...
		be ? "big" : "little",
		smashacmd_rd32 (data + 4, be), n, smashacmd_rd32 (data + 12, be));
	...
	for (u32 i = 0; i < n; i++)
	{
		...
		const u32 next = i + 1 < n
			? smashacmd_rd32 (data + 16 + (size_t)(i + 1) * 8 + 4, be)
			: (u32)size;
...
enumError DecodeSmashMTable_Text (FILE *out, const u8 *data, size_t size)
{
	...
	for (u32 i = 0; i < n; i++)
		fprintf (out, "%u | 0x%08x | 0x%08x\n",
			i, rd_be32 (data + (size_t)i * 4), rd_le32 (data + (size_t)i * 4));
	return ERR_OK;
}
