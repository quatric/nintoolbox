...
enumError create_ncer_xml (ccp source, ccp dest)
{
	...
	mxml_node_t *cell_node
		= mxmlFindElement (ncer_node, ncer_node, "cell", NULL, NULL, MXML_DESCEND_FIRST);
	for (uint i = 0; i < n_cells; i++)
	{
		if (!cell_node)
		{
		...
		if (index != i || count > 65535 - n_objects)
		{
			err = ERR_INVALID_DATA;
			break;
		}
		...
		mxml_node_t *obj_node
			= mxmlFindElement (cell_node, cell_node, "obj", NULL, NULL, MXML_DESCEND_FIRST);
		for (uint j = 0; j < count; j++)
		{
			...
			if (a0 > 0xffff || a1 > 0xffff || a2 > 0xffff)
			{
				err = ERR_INVALID_DATA;
				break;
			}
