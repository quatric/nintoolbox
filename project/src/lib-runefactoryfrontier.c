	...
	for (size_t i = RFF_HX_HEADER_SIZE; i <= size; i++)
	{
		...
		else
		{
			if (run_start != (size_t)-1 && i - run_start >= 4)
			{
				...
				{
					fprintf (f,
						"\n# embedded printable-ASCII strings (heuristic scan, likely entity/bone "
						"names)\n");
					printed = 1;
				}
...
enumError DecodeRFFHxtb_Text (FILE *f, const u8 *data, size_t size, size_t file_size)
{
	...
	fprintf (f,
		"table_region_size = 0x%x  # big-endian, == file_size - 0x20 in every sample seen\n",
		table_size);
	...
	{
		fprintf (
			f, "# entry table truncated/out-of-bounds/unexpected stride -- not decoded further\n");
		return ERR_OK;
	}

	fprintf (f, "\n# entry table (%u entries, 0x%x bytes each, starting at 0x%x)\n", count,
		entry_stride, RFF_HX_HEADER_SIZE);
...
int IsRFFHxcb (const u8 *data, size_t size, size_t file_size)
{
	(void)file_size;
	return is_hx_magic (data, size, "HXCB", 2);
}
int IsRFFHxmb (const u8 *data, size_t size, size_t file_size)
{
	(void)file_size;
	return is_hx_magic (data, size, "HXMB", 1);
}
int IsRFFHxhb (const u8 *data, size_t size, size_t file_size)
{
	(void)file_size;
	return is_hx_magic (data, size, "HXHB", 1);
}
int IsRFFHxgb (const u8 *data, size_t size, size_t file_size)
{
	(void)file_size;
	return is_hx_magic (data, size, "HXGB", 1);
}
int IsRFFHxtp (const u8 *data, size_t size, size_t file_size)
{
	(void)file_size;
	return is_hx_magic (data, size, "HXTP", 1);
}
...
enumError DecodeRFFHxGeneric_Text (FILE *f, const u8 *data, size_t size, size_t file_size)
{
	...
	fprintf (f,
		"# generic HX-family header; per-type record layout beyond the header is not "
		"reverse-engineered\n");
	...
	fprintf (f,
		"\n# body not reverse-engineered beyond the strings above -- see "
		"lib-runefactoryfrontier.h\n");
	return ERR_OK;
}
...
enum
{
	RFF_FBTI_HEADER_SIZE = 0x10,
	RFF_FBTI_ENTRY_SIZE = 0x08
};
...
enumError DecodeRFFFbti_Text (FILE *f, const u8 *data, size_t size, size_t file_size)
{
	...
	fprintf (
		f, "header_size = 0x%x  # big-endian, always 0x10 in every sample seen\n", header_size);
	...
	for (u32 i = 0; i < count; i++)
	{
		...
		fprintf (
			f, "section[%u]: offset = 0x%x  size = 0x%x  end = 0x%x", i, ofs, dlen, ofs + dlen);
	...
	fprintf (f,
		"\n# per-section payload content (beyond an embedded HX header, when present) not "
		"reverse-engineered\n");
	return ERR_OK;
}
