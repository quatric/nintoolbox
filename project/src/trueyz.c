...
#include <intrin.h>
static uint32_t TRUEYZ_CTZ32 (uint32_t x)
{
	unsigned long i;
	_BitScanForward (&i, x);
	return (uint32_t)i;
}
static uint32_t TRUEYZ_CLZ32 (uint32_t x)
{
	unsigned long i;
	_BitScanReverse (&i, x);
...
static uint32_t TRUEYZ_CTZ64 (uint64_t x)
{
	unsigned long i;
	_BitScanForward64 (&i, x);
	return (uint32_t)i;
}
static uint32_t TRUEYZ_CLZ64 (uint64_t x)
{
	unsigned long i;
	_BitScanReverse64 (&i, x);
	return 63u - (uint32_t)i;
}
#endif
#else
static uint32_t TRUEYZ_CTZ32 (uint32_t x)
 ...
 */
static TRUEYZ_ALWAYS_INLINE void writer_emit_match (
	yaz0_writer_t *w, uint32_t len, uint32_t distance)
{
	writer_ensure_group (w);
...
typedef char trueyz_scratch_size_check[(sizeof (trueyz_state_t) == TRUEYZ_SCRATCH_SIZE) ? 1 : -1];
#endif
 ...
 */
static TRUEYZ_ALWAYS_INLINE uint32_t matcher_longest_match (const trueyz_matcher_t *m,
	uint32_t position, uint32_t head, uint32_t previous_length, uint32_t chain_limit,
	uint32_t *match_position)
{
	...
	const uint8_t *limit
		= (position + MAX_MATCH < m->length) ? scan + MAX_MATCH : m->data + m->length;
...
static TRUEYZ_ALWAYS_INLINE int compress_core (const void *input, uint32_t length, void *output,
	uint32_t alignment, trueyz_state_t *restrict state)
{
	...
	while (position < length)
	{
		...
		{
			match_length = matcher_longest_match (
				&matcher, position, head, previous_length, chain_limit, &match_position);
		}
	...
	const void *input, int length, void *output, int alignment, void *scratch)
{
	...
	return compress_core (
		input, (uint32_t)length, output, (uint32_t)alignment, (trueyz_state_t *)scratch);
}
#else
static int TRUEYZ_ALWAYS_INLINE compress_static (
	const void *input, uint32_t length, void *output, uint32_t alignment)
{
