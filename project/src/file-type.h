...
typedef enum file_format_t
{
	...
	FF_ZMB, // 347 - Konami Bemani skinned character/prop model (.bin, Dance Dance Revolution: Winx
			// Club)
	...
	FF_CHNK, // 357 - Monster 4x4: Stunt Racer chunked container
			 // (.bnk/.d4l/.d4c/.gfx/.mec/.pmu/.pak/.ppx/.an4)
	...
	FF_MURAMASA_FCMP, // 370 - Muramasa FCMP compressed container (.mbs/.ftx/.esb/.nsb/.abf/.nms;
					  // payload not decoded)
	...
	FF_ZACKWIKI_SSD, // 383 - Zack & Wiki streamed ADPCM audio (.ssd; extension only, no confirmed
					 // structure)
	...
	FF_MERCURY_MAT, // 391 - Mercury Meltdown Revolution material float-record table (.mat;
					// magic-less)
	FF_MERCURY_NAV, // 392 - Mercury Meltdown Revolution navigation mesh table (.nav; magic-less,
					// header only)
	...
	FF_RFF_HXTB, // 400 - Rune Factory: Frontier "HX Table" entry directory (.hvt/.Hvt; entry table
				 // decoded)
	...
	FF_HTTYD_MTD, // 409 - DreamWorks How to Train Your Dragon chunk-tree container (.mtd; same
				  // layout as .RWS)
	FF_HTTYD_KRV, // 410 - DreamWorks How to Train Your Dragon gzip-wrapped localization string
				  // table (.KRV)
	FF_BERMUDA_MWT, // 411 - Bermuda Triangle "GDATAVERSION" resource envelope (.MWT; wraps a
					// Camelot GX texture bank)
	...
	FF_DIABOLIK_RES, // 420 - Diabolik: The Original Sin FAAFFAAF tagged-block resource container
					 // (.cfg/.gam/.loc/.ls/.rgn)
	FF_DIG, // 421 - Bomberman Land "cddata*.dig" streaming resource package (.dig; extract-only,
			// flat table variant)
	FF_BOTB_BAG, // 422 - Battle of the Bands ".bag" asset container (header + sub-resource manifest
				 // only)
	...
	FF_HDVOICE, // 425 - Tenchu: Shadow Assassins voice-line manifest (.hd; slot table + parameter
				// table, fully decoded)
	FF_XTD, // 426 - Genki "GTI Club: Supermini Festa!" car resource table (.unq.xtd; wraps raw
			// Nintendo TPL textures)
	FF_XMD, // 427 - Genki "GTI Club: Supermini Festa!" car model container (.mdl; wraps a
