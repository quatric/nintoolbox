...
#define CopyImage	CopyImage_Win32Unused // winuser.h's CopyImage() collides
					       // with wit's lib-sf.h CopyImage()
#define GetSystemMenu	GetSystemMenu_Win32Unused // winuser.h's GetSystemMenu()
						  // collides with wit's titles.h
...
#undef GetCurrentTime // windows.h's GetTickCount() alias shadows dclib's own
		       // GetCurrentTime(bool)
#undef SORT_DEFAULT // winnls.h's locale-sort constant collides with wit's
		     // lib-std.h SortMode_t enumerator of the same name
...
// mkstemps() (mkstemp() with a fixed-length suffix, e.g. ".xml") is a
// glibc/BSD extension; MinGW's mkstemp() (declared in <stdlib.h>) has no
// suffix support at all, so this fills the XXXXXX run itself and opens the
// file directly rather than composing it from mkstemp().
#include <fcntl.h>
#include <sys/stat.h>
static inline int dclib_mingw_mkstemps (char *template_, int suffixlen)
{
	size_t len = strlen (template_);
	if ((size_t)suffixlen > len)
		return -1;
	char *xend = template_ + len - suffixlen;
	char *x = xend;
	while (x > template_ && x[-1] == 'X')
		x--;
	if (x == xend)
		return -1;
	...
	{
		for (char *p = x; p < xend; p++)
			*p = chars[rand () % (sizeof (chars) - 1)];
		int fd = open (template_, O_RDWR | O_CREAT | O_EXCL | O_BINARY, 0600);
		if (fd >= 0)
			return fd;
	}
	return -1;
}
#define mkstemps dclib_mingw_mkstemps
...
// MinGW's struct stat has no st_blksize (no block-size concept in the
// classic Windows stat() model); the couple of call sites that read
// 'f->st.st_blksize' as an I/O chunk-size hint are patched to use this
// instead on MinGW.
#define DCLIB_MINGW_ST_BLKSIZE 4096
